...
************************************************/
int create_aggregate_table_entry_ex(int* row_num)
{
...
//select timestamp, a.flowpathinstance, tierindex, serverindex, appindex, urlindex, httpheadername, valuetype, valueint, httpheadervalue responsetime, btcategory
/**************************************
...
NVResultSet *nd_agg_parser(NVResultSet *data, int *numoutresultset, long timestamp)
{
  ...
  *numoutresultset = 1;
  ...
  for(i = 0; i < data->numRow; i++)
  {
    ...
    //char *httpheadername = nvGetValue(data, i, end_httpheadername)->value.str;
    int valuetype = atoi(nvGetValue(data, i, end_valuetype)->value.str);
    int valueint = atoi(nvGetValue(data, i, end_valueint)->value.str);
    int httpheadervalue = atoi(nvGetValue(data, i, end_httpheadervalue)->value.str);
    ...
    //Aggregate on the basis of storeid
    //If match found then update timing details
    for(j = 0; j < total_aggr_entries; j++)
    {    
      if(
        (aggrTableDetail[j].httpheadervalue == httpheadervalue) && (aggrTableDetail[j].btid == btID)
        &&
    ...
    int idx= create_aggregate_table_entry_ex(&rnum);
    ...
    //strcpy(aggrTableDetail[idx].httpheadername, httpheadername);
    aggrTableDetail[idx].httpheadervalue = httpheadervalue;
  ...
  { 
    ...
    setNVColumnNumber(&col[eout_httpheadervalue], aggrTableDetail[k].httpheadervalue);
