...
int rpc_name;
    ...
    ns_tr069_register_rfc(NULL, -1, NULL);
    //ns_tr069_register_rfc("192.168.1.66", 33333, "http://192.168.255.2/ns_cpe_url_mapping.html");
...
start_session_with_acs:
    ...
    rpc_name = ns_tr069_cpe_invoke_inform();  
    ...
    switch (rpc_name)
    {
        case NS_TR069_TRANSFER_COMPLETE:
            ns_start_transaction("TR069TransferComplete"); 
            rpc_name = ns_tr069_cpe_transfer_complete();
            ns_end_transaction("TR069TransferComplete", NS_AUTO_STATUS);
            break;
        //default:
    }

    ns_start_transaction("TR069InviteRpc");
    rpc_name = ns_tr069_cpe_invite_rpc();
    ...
    int rfc_ret;
    int rfc_wait_time;
    int periodic_inform_time =  ns_tr069_get_periodic_inform_time_ex(); // Seconds
