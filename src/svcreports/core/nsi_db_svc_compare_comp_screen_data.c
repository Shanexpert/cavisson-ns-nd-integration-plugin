...
#define HEADER "TestRun|SvcIndex|SvcCompName|Count|RespTime|Min|Max|Variance|CPFlagc\n"
...
static inline void fill_svc_data_structer(char *buf, svc_Data **svcData, int *max_alloc_index, int *used_index)
{
  ...
  (*svcData)[*used_index].SvcIndex = atoi(fields[0]);
  strcpy((*svcData)[*used_index].SvcName, fields[1]);
  (*svcData)[*used_index].Count = atoi(fields[2]);
  (*svcData)[*used_index].RespTime = atof(fields[3]);
  (*svcData)[*used_index].Min = atoi(fields[4]);
  (*svcData)[*used_index].Max = atoi(fields[5]);
  (*svcData)[*used_index].Variance = atof(fields[6]);
  (*svcData)[*used_index].CPFlag = atof(fields[7]);
                                              ...
                                              int t1_used_index, svc_Data * t2_svcData, int t2_max_alloc_index, int t2_used_index)
{
  ...
  for(i = 0; i < t1_used_index; i++)
  {
    ...
    if(j == t2_used_index)
    {
      fprintf(stdout, "%d|%d|%s|%d|%.3f|%d|%d|%.3f|%d\n"
                      "%d|%c|%c|%c|%c|%c|%c|%c|%c\n", 
                      trnum, t1_svcData[i].SvcIndex, t1_svcData[i].SvcName, t1_svcData[i].Count, 
                      t1_svcData[i].RespTime, t1_svcData[i].Min, t1_svcData[i].Max, t1_svcData[i].Variance, t1_svcData[i].CPFlag,
                      trnum2, '-', '-', '-', '-', '-', '-', '-', '-');
      continue;
    }
    ...
    fprintf(stdout, "%d|%d|%s|%d|%.3f|%d|%d|%.3f|%d\n"
                    "%d|%d|%s|%d|%.3f|%d|%d|%.3f|%d\n", 
                      trnum, t1_svcData[i].SvcIndex, t1_svcData[i].SvcName, t1_svcData[i].Count, 
                      t1_svcData[i].RespTime, t1_svcData[i].Min, t1_svcData[i].Max, t1_svcData[i].Variance, t1_svcData[i].CPFlag,
                      trnum2, t2_svcData[j].SvcIndex, t2_svcData[j].SvcName, t2_svcData[j].Count, 
                      t2_svcData[j].RespTime, t2_svcData[j].Min, t2_svcData[j].Max, t2_svcData[j].Variance, t2_svcData[j].CPFlag);
  }
  for(i = 0; i < t2_used_index; i++)
  {
    if(t2_svcData[i].usedOrNot)
    {
      fprintf(stdout, "%d|%c|%c|%c|%c|%c|%c|%c|%c\n"
                      "%d|%d|%s|%d|%.3f|%d|%d|%.3f|%d\n", 
                      trnum, '-', '-', '-', '-', '-', '-', '-', '-',
                      trnum2, t2_svcData[i].SvcIndex, t2_svcData[i].SvcName, t2_svcData[i].Count, 
                      t2_svcData[i].RespTime, t2_svcData[i].Min, t2_svcData[i].Max, t2_svcData[i].Variance, t2_svcData[i].CPFlag);
    }
  }
}
...
int main(int argc, char **argv)
{
  ...
  static struct option long_options[] = {
    ...
    {"svcindex2",      required_argument, 0, 0},
    ...
    switch (c) 
    {
      case 0:
        switch(option_index)
        {
          ...
          case 3: //svcinstance
            svcIndex = atoi(optarg); 
            break;

          case 4: //svcinstance
            svcIndex2 = atoi(optarg); 
            break;
  ...
  sprintf(cmd, "nsi_db_svc_get_comp_data");
  for (i = 1; i < argc; i++)
  {
    if(!strcmp(argv[i], "--testrun2") || !strcmp(argv[i], "--svcindex2"))
      i++;
    else
  ...
  for (i = 1; i < argc; i++)
  {
    if(!strcmp(argv[i], "--testrun") || !strcmp(argv[i], "--svcindex"))
      i++;
    else if(!strcmp(argv[i], "--testrun2"))
      sprintf(cmd, "%s --testrun", cmd);
    else if(!strcmp(argv[i], "--svcindex2"))
      sprintf(cmd, "%s --svcindex", cmd);
    else 
      sprintf(cmd, "%s %s", cmd, argv[i]);
  }
