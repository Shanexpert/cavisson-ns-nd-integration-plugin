...
void db_insert_flow()
{
  ...
  {
    db_log_result(tablename, "Connect to DB for insert", "-,-,-,-,-", 1);
    ns_set_tx_status("Connect_db_for_insert", 70);
    ns_db_odbc_close();
    return;
  }
  db_log_result(tablename, "Connect to DB for insert", "-,-,-,-,-", 0);
...
// INSERT
  ...
  // Executing insert query
    ...
    sprintf(qstr, "insert into %s values (1,'bedsheet', 'home', 2, 10);", tablename); 
    if(ns_db_execute_direct(stmt, qstr) == -1)
    {
      ns_set_tx_status("Run_insert", NS_TX_ERROR);
      fprintf(stderr, "Error in executing query: [%s]\n", qstr);
      db_log_result(tablename, "Insert Row", "-,-,-,-,-", 1);
    }
    else 
      db_log_result(tablename, "Insert Row", "-,-,-,-,-", 0);
// End of 1st INSERT 
