...
//struct passwd *start_user_info;
//char *user_buf = NULL;
//int start_user_id = getuid();
//start_user_info = getpwuid(start_user_id);
//start_user_buf =  user_info->pw_name;

//if(!strcmp(start_user_buf,"root"))
  
extern void exit_on_sig_alarm(int sig);

static void usage()
{
  fprintf(stderr, "Usage:\n    nsu_resume_test -t <testrun> [ -i < time in seconds>]\n");
  exit(-1);
}

int main(int argc, char *argv[])
{
  int sock_fd, t_flag = 0, u_flag = 0, r_flag = 0, trnum, ns_uid, my_user_id;
  //int duration = 0;
  ...
  while ((c = getopt(argc, argv, "i:t:u:r:")) != -1) {
    switch(c) {
  ...
  { 
    fprintf(stderr, "Role %s is not authorized to resume the test\n", role_buf);
    exit(-1);
  }
  ...
  {
    fprintf(stderr, "User '%s' is not authorized to resume test TR%d\n", user_buf,trnum);
    exit(-1);
  }
