/***************************************************************************
 Name		: ns_socket_tcp_client_rpt.c 
 ...
 Design		:
 Author(s)	: Manish Mishra, 30 July 2020
 Mod. Hist.	:
***************************************************************************/
...
//TCP Client
#ifndef CAV_MAIN
int g_tcp_client_avg_idx = -1;                           // Index into avgtime
SocketClientAvgTime *g_tcp_client_avg = NULL;
#else
__thread int g_tcp_client_avg_idx = -1;
__thread SocketClientAvgTime *g_tcp_client_avg = NULL;
#endif
int g_tcp_client_cavg_idx = -1;                          // Index into cavgtime
SocketClientCAvgTime *g_tcp_client_cavg = NULL;

int g_tcp_client_rpt_group_idx = -1;                     // Index into Group_Info table 
TCPClientRTGData *g_tcp_client_rtg_ptr = NULL;           // RTG pointer for TCP Client 

//TCP Server
int g_tcp_server_avg_idx = -1;                           // Index into avgtime
int g_tcp_server_cavg_idx = -1;                          // Index into cavgtime
SocketClientAvgTime *g_tcp_server_avg = NULL;
SocketClientCAvgTime *g_tcp_server_cavg = NULL;

int g_tcp_server_rpt_group_idx = -1;                     // Index into Group_Info table 
TCPClientRTGData *g_tcp_server_rtg_ptr = NULL;           // RTG pointer for TCP Client 


//Function Definitions ---------------
void update_avgtime_size4socket_tcp_client() 
{
  int socket_avgsize = sizeof(SocketClientAvgTime);

  NSDL2_SOCKETS(NULL, NULL, "Method Called, protocol_enabled = %0x, "
               "g_avgtime_size = %d, g_tcp_client_avg_idx = %d, "
               "SocketAvgTime size = %d",
                global_settings->protocol_enabled, g_avgtime_size, 
                g_tcp_client_avg_idx, socket_avgsize);

  if (IS_TCP_CLIENT_API_EXIST) 
  {
    g_tcp_client_avg_idx = g_avgtime_size;
  ...
  NSDL2_SOCKETS(NULL, NULL, "AvgIndex for Sockect API done, g_avgtime_size = %d, "
           "g_tcp_client_avg_idx = %d",
            g_avgtime_size, g_tcp_client_avg_idx);
}

void update_cavgtime_size4socket_tcp_client() 
{
  #ifndef CAV_MAIN
  ...
  NSDL2_SOCKETS(NULL, NULL, "Method Called, protocol_enabled = %0x, "
               ...
               "SocketAvgTime size = %d",
                ...
                g_tcp_client_cavg_idx, socket_cavgsize);

  if (IS_TCP_CLIENT_API_EXIST) 
  {
    g_tcp_client_cavg_idx = g_cavgtime_size;
  ...
  NSDL2_SOCKETS(NULL, NULL, "CAvgIndex for Sockect API done, g_avgtime_size = %d, "
           "g_tcp_client_avg_idx = %d",
            g_cavgtime_size, g_tcp_client_cavg_idx);
  #endif
}

void set_avgtime_ptr4socket_tcp_client() 
{
  if (IS_TCP_CLIENT_API_EXIST) 
  {
    g_tcp_client_avg = (SocketClientAvgTime *)((char *)average_time + g_tcp_client_avg_idx);
    NSDL2_SOCKETS(NULL, NULL, "After setting g_tcp_client_avg = %p, g_tcp_client_avg_idx = %d", 
      g_tcp_client_avg, g_tcp_client_avg_idx);
  } 
  else 
  {
    NSDL2_SOCKETS(NULL, NULL, "Socket Protocol is disabled.");
    g_tcp_client_avg = NULL;
  }
}

void ns_socket_client_init_avgtime(avgtime *avgbuf, int avg_idx)
{
  SocketClientAvgTime *socket_avg;

  NSDL2_SOCKETS(NULL, NULL, "[SocketStats] init avgtime for socket client");
  socket_avg = (SocketClientAvgTime *)((char*)avgbuf + avg_idx); 

  socket_avg->dns_time.min = MAX_VALUE_4B_U; 
  socket_avg->conn_time.min = MAX_VALUE_4B_U; 
  socket_avg->ssl_time.min = MAX_VALUE_4B_U; 
  socket_avg->send_time.min = MAX_VALUE_4B_U; 
  socket_avg->recv_fb_time.min = MAX_VALUE_4B_U; 
  socket_avg->recv_time.min = MAX_VALUE_4B_U; 
}

void fill_tcp_client_avg(VUser *vptr, unsigned char metric, u_ns_8B_t value)
{
  SocketClientAvgTime *group_avg_ptr;

  NSDL2_SOCKETS(NULL, NULL, "Method called, [SocketStats] g_tcp_client_avg = %p, "
    "metric = %d, value = %lld", 
    g_tcp_client_avg, metric, value);

  CHILD_FILL_SOCKET_AVGTIME(g_tcp_client_avg, metric, value);
  
  if(SHOW_GRP_DATA)
  {
    group_avg_ptr = (SocketClientAvgTime *)((char *)((char *)average_time + 
                      ((vptr->group_num + 1) * g_avg_size_only_grp)) + g_tcp_client_avg_idx);
  ...
  Purpose	: Copy socket API data from avgtime/cavgtime to msg_data_ptr(RTG)
                  ...
                  Test Metrics
                    |-> TCP Client
                          |-> Overall
                                |-> Group-1
                                      |-> TCP Client Connections Initiated
                                      |-> ...
                                |-> Group-2
                                      |-> ...
                          |-> Generator-1
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
void fill_tcp_client_gp(avgtime **g_avg, cavgtime **g_cavg)
{
  ...
  for (group_vect_idx = 0; group_vect_idx < sgrp_used_genrator_entries + 1; group_vect_idx++)
  {
    ...
    {
      NSDL2_SOCKETS(NULL, NULL, "[SocketStats] group_idx = %d, g_tcp_client_avg_idx = %d", 
          sgrp_idx, g_tcp_client_avg_idx);
      
      avg_ptr = (SocketClientAvgTime *)((char*)((char *)g_avg[group_vect_idx] + 
                  (sgrp_idx *g_avg_size_only_grp)) + g_tcp_client_avg_idx);

      cavg_ptr = (SocketClientCAvgTime *)((char*)((char *)g_cavg[group_vect_idx] + 
                   (sgrp_idx *g_cavg_size_only_grp)) + g_tcp_client_cavg_idx);
...
void print_tcp_client_throughput(FILE *fp1, FILE *fp2, int is_periodic, avgtime *avg, cavgtime *cavg)
{
  ...
  avg_ptr = (SocketClientAvgTime *)((char *)avg + g_tcp_client_avg_idx);
  cavg_ptr = (SocketClientCAvgTime *)((char *)cavg + g_tcp_client_cavg_idx);

  if (is_periodic) 
  {
    tot_tcp_rx = avg_ptr->tot_bytes_recv;
    tot_tcp_tx = avg_ptr->tot_bytes_sent;
    
    duration = (double)((double)(global_settings->progress_secs)/1000.0);
    tcp_rx = ((double)tot_tcp_rx)/(duration*128.0);
    tcp_tx = ((double)tot_tcp_tx)/(duration*128.0);
  ...
  if(global_settings->show_initiated)
    fprint2f(fp1, fp2, "    TCP Client hit rate (per sec): Initiated=%.3f completed=%.3f Success=%.3f\n",
                          hit_initited_rate, hit_tot_rate, hit_succ_rate);
  else
    fprint2f(fp1, fp2, "    TCP Client hit rate (per sec): Total=%.3f Success=%.3f\n", hit_tot_rate, hit_succ_rate);

  fprint2f(fp1, fp2, "    TCP Client (Kbits/s): TCP(Rx=%'.3f Tx=%'.3f)\n",
                          tcp_rx, tcp_tx);

  fprint2f(fp1, fp2, "    TCP Client (Total Bytes): TCP(Rx=%'llu Tx=%'llu)\n",
                          tot_tcp_rx, tot_tcp_tx);

  fprint2f(fp1, fp2, "    TCP Client connections: Current=%llu Rate/s(Open=%.3f Close=%.3f) Total(Open=%'llu Close=%'llu)\n",
