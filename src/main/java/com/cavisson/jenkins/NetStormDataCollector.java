...
public class NetStormDataCollector {
  ...
  public NetStormDataCollector(final NetStormConnectionManager connection, final Run build, int TestRun, String testMode, String testCycleNumber) 
  {
  ...
  public NetStormReport createReportFromMeasurements(PrintStream logger, FilePath fp) throws Exception
  {
    ...
    try
    {
    	...
    	if(!isNDE && !isIntegrated)
    	{
           ...
           status  = connection.fetchMetricData(connection, METRIC_PATHS, durationInMinutes+"", groupId, graphId, TEST_RUN, TEST_MODE, logger, test_cycle_number, build);
    	}
        else if(isIntegrated && !isNDE)
        {
    	  ...
    	  status = nsNdConnection.fetchMetricData(nsNdConnection,METRIC_PATHS,durationInMinutes, groupId, graphId,TEST_RUN, TEST_MODE, fp);
