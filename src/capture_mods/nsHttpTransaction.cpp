...
#ifdef CAVISSON
    if (!cav_string_buf) {
        cav_string_buf = (char*) malloc(*bytesWritten);
        cav_string_length = *bytesWritten;
        memcpy(cav_string_buf, buf, *bytesWritten);
    } else {
        cav_string_buf = (char*) realloc( (void *) cav_string_buf, *bytesWritten + cav_string_length);
        memcpy(cav_string_buf + cav_string_length, buf, *bytesWritten);
        cav_string_length += *bytesWritten;
    }
#endif /* CAVISSON */
