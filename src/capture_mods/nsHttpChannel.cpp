...
#include <sys/wait.h>
char debug_buf[1000];
int cavisson_http_mod_fd = -1;
int cavisson_det_http_mod_fd = -1;
#endif /*CAVISSON*/
...
nsHttpChannel::Connect(PRBool firstTime)
{
    ...
    if (mCacheEntry) {
        cout << "There is a cache entry " << endl;
        ...
        if (mCachedContentIsValid) {
            cout << "Cache content is valid" << endl;
    ...
    if (getenv("CAVISSON_TOGGLE") && (!strcmp(getenv("CAVISSON_TOGGLE"), "1"))) {
        ...
        PRUint32 amount_read;
        char* string_buffer;
        nsresult rv;
        nsISeekableStream* seek_interface;
        
        cav_post_content = NULL;

        if (mTransaction->GetUploadStream() != nsnull) {
            rv = mTransaction->GetUploadStream()->Available(&content_length);
            if (NS_FAILED(rv)) {
                return rv;
            }

            string_buffer = (char*) malloc((content_length+2) * sizeof(char));
            
            rv = mTransaction->GetUploadStream()->Read(string_buffer, content_length, &amount_read);
            if (NS_FAILED(rv)) {
                return rv;
            }

            if (amount_read > content_length) {
                fprintf(stderr, "nsHttpChannel::Connect(): Error in reading the post content.  The amount read is more than the content length\n");
                free(string_buffer);
            } else {
                //string_buffer[amount_read] = '\n';
                //string_buffer[amount_read+1] = '\0';
                string_buffer[amount_read] = '\0';
                
                cav_post_content = string_buffer;
                
                /* reading the upstream data will set the read marker to end of the upstream */
                /* we must set the marker back to the beginning of the read stream */
                rv = mTransaction->GetUploadStream()->QueryInterface(NS_GET_IID(nsISeekableStream), (void **) &seek_interface);
                
                if (NS_SUCCEEDED(rv)) {
                    rv = seek_interface->Seek(seek_interface->NS_SEEK_SET, (PRInt32) 0);
                    if (NS_FAILED(rv))
                        return rv;
                }
                
                NS_IF_RELEASE(seek_interface);
            }
        }
    }
#endif /* CAVISSON */
...
nsHttpChannel::ProcessResponse()
{
    ...
    char dump_file_index[1024];
    char dump_file_base[1024];
    char *ptr, *ptr1, *ptr2;

#ifdef CAVISSON
    int first_url;
    int is_main_url = 0;
    static FILE * cav_dump_index_fp = NULL;
    static int page_num;

    if (getenv("CAVISSON_FIRST_URL") && (!strcmp(getenv("CAVISSON_FIRST_URL"), "1"))) {
            //CloseCacheEntry(NS_ERROR_ABORT);
            first_url = 1;
    } else {
            first_url = 0;        
    }
    // Dump the request into a file
    ptr = getenv("NS_WORK_DIR");
   if (ptr) {
	strcpy(dump_file_name, ptr);
   } else {
	strcpy(dump_file_name, "/home/netstorm/work/scripts");
   }
   strcpy(dump_file_base, "/scripts");
   strcat(dump_file_name, "/");
   strcat(dump_file_base, "/");
   ptr = getenv("NS_SCRIPT_NAME");
    if (ptr) {
	strcat(dump_file_name, ptr);
	strcat(dump_file_base, ptr);
    } else {
	//Control should never come here
	strcat(dump_file_name, "tmp");
	strcat(dump_file_base, "tmp");
    }

    strcat (dump_file_name, "/dump");
    strcat (dump_file_base, "/dump");

    if (!cav_dump_index_fp) {

	sprintf(dump_file_index, "mkdir -p %s", dump_file_name);
	system(dump_file_index);
    	sprintf(dump_file_index, "%s/index", dump_file_name);
	cav_dump_index_fp = fopen(dump_file_index, "w");
	if (!cav_dump_index_fp) {
		printf("Unable to open cav dump index file\n");
	}
	page_num = 0;
    }


    //if (cavisson_http_mod_fd == -1) 
    {
        char* url_fd;
        url_fd = getenv("CAVISSON_FD");
        if (url_fd) {
            cavisson_http_mod_fd = atoi(url_fd);
            cout << "processresponse: cavisson_http_mod_fd is " <<  cavisson_http_mod_fd << endl;
        }
    }

    //if (cavisson_det_http_mod_fd == -1) 
    {
        char* url_fd;
        url_fd = getenv("CAVISSON_DETAIL_FD");
        if (url_fd) {
            cavisson_det_http_mod_fd = atoi(url_fd);
            cout << "processresponse: cavisson_det_http_mod_fd is " << cavisson_det_http_mod_fd << endl;
        }
    }

    if (getenv("CAVISSON_TOGGLE") && (!strcmp(getenv("CAVISSON_TOGGLE"), "1"))) {
        int string_length;
        nsCString short_headers;
        int filepipe[2];
        ...
        int tx_comp_ratio = 1;
        int status;
        static int num_embedded;
	char url_buf[8192];
	char host_buf[128];

	static int cav_dump_fd =-1;
	static int cav_dump_fd2 =-1;
	static int cav_dump_fd3 =-1;
	static int last_redirect = 0;
	int cav_fd = -1;
	char * base_marker;
	char dump_data[strlen(mTransaction->GetRequest().get()) + 1];
        ...
        if (child_pid == 0) {
            ...
            write(filepipe[1], mTransaction->GetRequest().get(), strlen(mTransaction->GetRequest().get()));

		    if (cav_dump_fd3 == -1) {
		    	cav_dump_fd3 = open("/tmp/dump3", O_CREAT|O_RDWR, 0666);
                    	write(cav_dump_fd3, "Adding get START\n", strlen ("Adding get START\n"));
		    }
	    if (httpStatus/100 == 3) { //If redirect
		if (first_url)
		    last_redirect = 1;
		...
		if (first_url || last_redirect) {
	        ...
	        //Get the Host name and URL of the request
	        strcpy (dump_data, mTransaction->GetRequest().get());
	        ptr = dump_data;
	        //ptr = (char *) mTransaction->GetRequest().get();
                //write(cav_dump_fd3, "Adding get copy\n", strlen ("Adding get copy\n"));
                //write(cav_dump_fd3, ptr, strlen(ptr));
	        ptr1 = strstr(ptr, "\r\n");
                //write(cav_dump_fd3, "Adding ptr\n", strlen ("Adding ptr\n"));
                //write(cav_dump_fd3, ptr1?ptr1:"-", ptr1?strlen(ptr1):1);
		if (ptr1) {
	            strncpy(url_buf, ptr, ptr1-ptr);
	            url_buf[ptr1-ptr] = '\0';
                    //write(cav_dump_fd3, "Adding url_buf\n", strlen ("Adding url_buf\n"));
                    //write(cav_dump_fd3, url_buf, strlen (url_buf));
	            ptr = strtok(url_buf, " :;?"); //ptr has method
	            ptr = strtok(NULL, " :;?"); //ptr has URI's base name 
                    //write(cav_dump_fd3, "Adding url\n", strlen ("Adding url\n"));
                    //write(cav_dump_fd3, ptr, strlen (ptr));
	            ptr1 +=2; //move past \r\n
	    	    while (1) {
                    	//write(cav_dump_fd3, "Adding ptr1\n", strlen ("Adding ptr1\n"));
                    	//write(cav_dump_fd3, ptr1, strlen (ptr1));
	    		ptr2 = strstr(ptr1, "\r\n");
			if (!ptr2) break;
                    	//write(cav_dump_fd3, "Adding ptr2\n", strlen ("Adding ptr2\n"));
                    	//write(cav_dump_fd3, ptr2?ptr2:"-", ptr2?strlen (ptr2):1);
			if (ptr1 == ptr2) break;
			if (strncasecmp (ptr1, "Host:", 5) == 0) {
		    	    ptr1 += 5;
		    	    while (isspace(*ptr1)) ptr1++;
		    	    strncpy (host_buf, ptr1, ptr2-ptr1);
		    	    host_buf[ptr2-ptr1] = '\0';
                    	    //write(cav_dump_fd3, "Adding host\n", strlen ("Adding host\n"));
                    	    //write(cav_dump_fd3, host_buf, strlen (host_buf));
			}
	        	ptr1 = ptr2 + 2; //move past \r\n
	            }
	            strcat (dump_file_name, "/");
	            strcat (dump_file_base, "/");
	            strcat (dump_file_name, host_buf);
	            strcat (dump_file_base, host_buf);
	            //strcat (dump_file_name, "/");
		    ptr1 = getenv("CAVISSON_LAST_PAGE");
		    ptr2 = rindex(ptr, '/');
		    if (is_main_url && ptr1 && ptr2) {
			char root_path[128], *tptr;
			int first_depth = 1;

		        ptr2++;  *ptr2 = '\0';
	                //strcat (dump_file_name, ptr2);
	                //strcat (dump_file_base, ptr2);
			//ptr has the base url
	                strcat (dump_file_name, ptr);
	                strcat (dump_file_base, ptr);
	                //strcat (dump_file_name, "/");
	                strcat (dump_file_name, ptr1);
	                strcat (dump_file_base, ptr1);
			tptr = index(ptr, '/');
			root_path[0] = '\0';
			while (tptr) {
			    tptr++;
			...
			//dump_file_base is not logged anymore, just url path
			fprintf (cav_dump_index_fp, "%d,%s,%s,%s,%s\n", page_num, ptr1, ptr, host_buf, root_path);
			fflush(cav_dump_index_fp);
		    } else {
			if (is_main_url) {
			    if (!ptr1) 
                    		write(cav_dump_fd3, "Expected CAVISSON_LAST_PAGE defined\n", strlen("Expected CAVISSON_LAST_PAGE defined\n"));
			    if (!ptr2) 
                    		write(cav_dump_fd3, "Expected / in the path\n", strlen("Expected / in the path\n") );
			}
	                strcat (dump_file_name, ptr);
	            	if (ptr[strlen(ptr) -1] == '/')
			    strcat (dump_file_name, "index");
		    }
	            base_marker = rindex (dump_file_name, '/');
	            *base_marker = '\0';
	            sprintf (url_buf, "mkdir -p %s", dump_file_name);
                    //write(cav_dump_fd3, "Adding mkdir\n", strlen ("Adding mkdir\n"));
                    //write(cav_dump_fd3, url_buf, strlen (url_buf));
	            system(url_buf);
	            *base_marker = '/';
	            cav_fd = open(dump_file_name, O_CREAT|O_RDWR|O_EXCL, 0666);
                    write(cav_dump_fd3, "Adding fname\n", strlen ("Adding fname\n"));
                    write(cav_dump_fd3, dump_file_name, strlen (dump_file_name));
		}
	    }
	    
                    //write(cav_dump_fd3, "Adding get\n", strlen ("Adding get\n"));
             	    //write(cav_dump_fd3, mTransaction->GetRequest().get(), strlen(mTransaction->GetRequest().get()));
        ...
        if (mTransaction->cav_string_buf) {
            ...
            if (child_pid == 0) {
                ...
                sprintf(write_file_fd, "%d", filepipe[1]);
                sprintf(write_compression_fd, "%d", compressionpipe[1]);
                execl("./v42/compact", "./v42/compact", "-T", "10", "-i", read_file_fd, "-g", write_file_fd, "-h", write_compression_fd, "-q", "-f", "/dev/null", NULL);
            } else {
                int status;
                char comp_buff[16];
                write(filepipe[1], mTransaction->cav_string_buf, mTransaction->cav_string_length);
                if (cav_post_content) {
                    write(filepipe[1], cav_post_content, strlen(cav_post_content));
		    if (cav_dump_fd2 == -1) {
		    	cav_dump_fd2 = open("/tmp/dump2", O_CREAT|O_RDWR, 0666);
                    	write(cav_dump_fd2, "Adding Post START\n", strlen ("Adding Post START\n"));
		    }
                    write(cav_dump_fd2, "Adding Post\n", strlen ("Adding Post\n"));
                    write(cav_dump_fd2, cav_post_content, strlen(cav_post_content));
		}
                close(filepipe[1]);
                close(filepipe[0]);
                read(compressionpipe[0], comp_buff, 16);
		...
		if (cav_dump_fd == -1) {
		    cav_dump_fd = open("/tmp/dump", O_CREAT|O_RDWR, 0666);
		}
                write(cav_dump_fd, "\nAdding Entry: ", strlen ("\nAdding Entry: "));
                write(cav_dump_fd, mTransaction->cav_string_buf, mTransaction->cav_string_length);
		
		if (cav_fd != -1) {
		    int chunked = 0;

		    ptr = mTransaction->cav_string_buf;
#if 0
		    ptr1 = strstr (ptr, "\r\n\r\n");
                    //write(cav_fd, mTransaction->cav_string_buf, mTransaction->cav_string_length);
		    if (ptr1) {
		        ptr1 += 4;
                        write(cav_fd, ptr1, mTransaction->cav_string_length - (ptr1-ptr));
		    }
#endif
		    ptr1 = ptr;
	    	    while (1) {
			...
			if (strncasecmp (ptr1, "Transfer-Encoding:", 18) == 0) {
		    	    ptr1 += 18;
		    	    while (isspace(*ptr1)) ptr1++;
			    if (strncasecmp (ptr1, "chunked", 7) == 0) {
				chunked = 1;
			    }
                    	    //write(cav_dump_fd3, "Adding host\n", strlen ("Adding host\n"));
                    	    //write(cav_dump_fd3, host_buf, strlen (host_buf));
			...
			int left_len = mTransaction->cav_string_length - (ptr1-ptr);
			...
			while (1) {
			    ...
			    if (!ptr2) break;
			    *ptr2 = '\0';
                            write(cav_dump_fd, ptr1, strlen (ptr1));
			    chunk_len = strtoul(ptr1, NULL, 16);
			    ptr2 = ptr2+2; //Get past \r\n
			    //if ((chunk_len == 0) && (strncmp(ptr2, "\r\n", 2) == 0))
			    if (chunk_len == 0)
			 	break; //terninating chunk
			    left_len -= (ptr2-ptr1);
			    if (left_len < chunk_len) break;
                            write(cav_fd, ptr2, chunk_len);
			    ptr1 = ptr2 + chunk_len + 2; //for chunk endinging in \r\n
		    ...
		    close(cav_fd);
                ...
                rx_comp_ratio = atoi(comp_buff);
                if (rx_comp_ratio < 0)
                    rx_comp_ratio = 0;
                free(mTransaction->cav_string_buf);
                mTransaction->cav_string_buf = NULL;
                mTransaction->cav_string_length = 0;
                waitpid(child_pid, &status, 0);
            }
        }
	...
	if (first_url)
        ...
        if (first_url && cavisson_det_http_mod_fd) {
	    if (httpStatus/100 == 3) //If rediect
                sprintf(write_buffer, "CMP_RAT=%d/%d;REDIRECT=YES\n", tx_comp_ratio, rx_comp_ratio);
	    else
                sprintf(write_buffer, "CMP_RAT=%d/%d\n", tx_comp_ratio, rx_comp_ratio);
            string_length = strlen(write_buffer);
            if (write(cavisson_det_http_mod_fd, write_buffer, string_length) != string_length)
                cout << "CAVISSON: error in writing to detail url file" << endl;
        } else if (cavisson_det_http_mod_fd) {
            
            string_length = strlen("---- ");
            if (write(cavisson_det_http_mod_fd, "---- ", string_length) != string_length)
                cout << "CAVISSON: error is writing to detail url file" << endl;
            
            if (mConnectionInfo->UsingSSL()) {
                string_length = strlen("HTTPS ");
                if (write(cavisson_det_http_mod_fd, "HTTPS ", string_length) != string_length) 
                    cout << "CAVISSON: error is writing to detail url file" << endl;
            }
                                                 
            sprintf(write_buffer, "TX_RAT:%d ", tx_comp_ratio);
            string_length = strlen(write_buffer);
            
            if (write(cavisson_det_http_mod_fd, write_buffer, strlen(write_buffer)) != string_length)
                cout << "CAVISSON: error is writing to detail url file" << endl;
            
            sprintf(write_buffer, "RX_RAT:%d\n", rx_comp_ratio);
            string_length = strlen(write_buffer);
        ...
        if (cavisson_http_mod_fd && cavisson_det_http_mod_fd) {
            mRequestHead.GetShortRequest(short_headers, first_url?cavisson_http_mod_fd:-1, cavisson_det_http_mod_fd, mConnectionInfo->UsingSSL());

            if (!first_url) {
                string_length = strlen(short_headers.get());
                
                if (write(first_url?cavisson_http_mod_fd:cavisson_det_http_mod_fd, short_headers.get(), string_length) != string_length)
                    cout << "CAVISSON: error in writing to detail url file" << endl;
            }
            
            if (cav_post_content) {
                string_length = strlen(cav_post_content);

                if (first_url) {
		char * ptr, *ptr1;
                                                                                
	            if (httpStatus/100 == 3) { //If rediect
                    	if (write(cavisson_http_mod_fd, "          REDIRECT=YES,\n", strlen("          REDIRECT=YES,\n")) != strlen("          REDIRECT=YES,\n"))
                            cout << "CAVISSON: error in writing to capture url file" << endl;
		    }
    		    ptr = cav_post_content;
    		    while (1) {
        		ptr1 = strstr (ptr, "\r\n");
        		if (!ptr1) {
                	    printf("EOF:BODY is <%s>\n", ptr);
                	    break;
        		}
        		* ptr1 = '\0';
        		ptr1++;ptr1++;
        		if (!strlen(ptr)) {
                	    ptr = ptr1;
                	    printf("Last Header found. Body is <%s>\n", ptr);
                	    break;
        		}
			//Header  found
        		//printf("HEADER is <%s>\n", ptr);
			...
			//Log the header
                    	if (write(cavisson_http_mod_fd, "          HEADER=", strlen("          HEADER=")) != strlen("          HEADER="))
                            cout << "CAVISSON: error in writing to capture url file" << endl;
                    	if (write(cavisson_http_mod_fd, ptr, strlen(ptr)) != strlen(ptr))
                            cout << "CAVISSON: error in writing to capture url file" << endl;
                    	if (write(cavisson_http_mod_fd, ",\n", 2) != 2)
                            cout << "CAVISSON: error in writing to capture url file" << endl;
                                                                                
        		ptr = ptr1;
   		    }

                    if (write(cavisson_http_mod_fd, "          BODY=", strlen("          BODY=")) != strlen("          BODY="))
                        cout << "CAVISSON: error in writing to capture url file" << endl;
                    //if (write(cavisson_http_mod_fd, cav_post_content, string_length) != string_length)
                    if (write(cavisson_http_mod_fd, ptr, strlen(ptr)) != strlen(ptr))
                        cout << "CAVISSON: error in writing to capture url file" << endl;
                    if (write(cavisson_http_mod_fd, ",\n", 2) != 2)
                        cout << "CAVISSON: error in writing to capture url file" << endl;
                } else {
                    if (write(cavisson_det_http_mod_fd, cav_post_content, string_length) != string_length)
                        cout << "CAVISSON: error in writing to detail url file" << endl;
                }
                
                free(cav_post_content);
            } else {
                if (!first_url) {
                
                    string_length = strlen("\r\n");
                    
                    if (write(cavisson_det_http_mod_fd, "\r\n", string_length) != string_length)
                        cout << "CAVISSON: error in writing to detail url file" << endl;
                }
            }
            ...
            if (first_url) {
                if (setenv("CAVISSON_FIRST_URL", "0", 1) == -1)
                    cout << "CAVISSON: Error in setting env. variable" << endl;
                num_embedded = 0;
            }
            else {
                num_embedded++;
            ...
            sprintf(num_buf, "%d", num_embedded);

            if (setenv("CAVISSON_NUM_EMBED", num_buf, 1) == -1)
                cout << "CAVISSON: Error in setting env. variable" << endl;
        }
    }
#endif /* CAVISSON */
...
#ifdef CAVISSON
    /*
    nsCString cavisson_header;
    
    mResponseHead->Flatten(cavisson_header, 0);
    ...
    if (getenv("CAVISSON_TOGGLE") && (!strcmp(getenv("CAVISSON_TOGGLE"), "1")) && (getenv("CAVISSON_RESPONSE_DUMP"))) {
        if (first_url?cavisson_http_mod_fd:cavisson_det_http_mod_fd) {
            int string_length;

            string_length = strlen(cavisson_header.get());

            if (write(first_url?cavisson_http_mod_fd:cavisson_det_http_mod_fd, cavisson_header.get(), string_length) != string_length)
                cout << "CAVISSON: error is writing to detail url file" << endl;
        }
        }*/
#endif /* CAVISSON */
...
nsHttpChannel::CheckCache()
{
    ...
    if (!mCacheEntry || !(mCacheAccess & nsICache::ACCESS_READ)) {
        cout << "there is no cache entry open for reading" << endl;
        return NS_OK;
    }
...
nsHttpChannel::OnStartRequest(nsIRequest *request, nsISupports *ctxt)
{
    ...
    //if (NS_SUCCEEDED(mStatus) && ((!mCacheReadRequest) || (getenv("CAVISSON_FIRST_URL") && (!strcmp(getenv("CAVISSON_FIRST_URL"), "1")))) && mTransaction) {
