...
int cavisson_doc_mod_fd = -1;
static char last_cavisson_sess_name[256];
int cavisson_det_doc_mod_fd = -1;
int cavisson_c_doc_mod_fd = -1;
int cavisson_h_doc_mod_fd = -1;
//static int cavisson_first = 1;
static int define_cnt = 0;

//KNQ: is this hash fun provided by mozilla , how it works - NSPR LIBs
PLHashTable* cavisson_hash_table=NULL;

static void * PR_CALLBACK
CavissonAllocTable(void *pool, PRSize size)
{
    return malloc(size);
}

static void PR_CALLBACK
CavissonFreeTable(void *pool, void *item)
{
    free(item);
}

static PLHashEntry * PR_CALLBACK
CavissonAllocEntry(void *pool, const void *key)
{
    return (PLHashEntry *)malloc(sizeof(PLHashEntry));
}

static void PR_CALLBACK
CavissonFreeEntry(void* pool, PLHashEntry *he, PRUintn flag) {
...
PLHashAllocOps CavissonAllocOps = {CavissonAllocTable, CavissonFreeTable, CavissonAllocEntry, CavissonFreeEntry};

//KQ: when is this sig sent?
void cavisson_signal_handler(int signum) {
    char* url_fd;
    char* url_detail_fd;
    char* c_fd;
    char* h_fd;
    ...
    if (signum == SIGHUP) {
        ...
        if (url_fd)
            cavisson_doc_mod_fd = atoi(url_fd);
        url_detail_fd = getenv("CAVISSON_DETAIL_FD");
        if (url_detail_fd)
            cavisson_det_doc_mod_fd = atoi(url_detail_fd);
        c_fd = getenv("CAVISSON_C_FD");
        if (c_fd)
            cavisson_c_doc_mod_fd = atoi(c_fd);
        h_fd = getenv("CAVISSON_H_FD");
        if (h_fd)
            cavisson_h_doc_mod_fd = atoi(h_fd);
    }
}
#endif /* CAVISSON */
...
//#ifdef CAVISSON
    ...
    if (cavisson_first) {
        ...
        /*        if (signal(SIGHUP, cavisson_signal_handler) == SIG_ERR)
                  cout << "CAVISSON: Error in setting signal handler" << endl;*/

        /*        if ((file_name = getenv("CAVISSON_PID_FILE"))) {
            if ((file_fd = open(file_name, O_RDWR| O_CREAT | O_TRUNC, 0666))) {
                sprintf(buf, "%d", getpid());
                if (write(file_fd, buf, strlen(buf)) != (int) strlen(buf))
                    cout << "CAVISSON: Error in writing to pid file" << endl;
                close(file_fd);
            }
            unsetenv("CAVISSON_PID_FILE");
        }
        
        if ((file_name = getenv("CAVISSON_URL_FILE"))) {
            if ((cavisson_doc_mod_fd = open(file_name, O_RDWR | O_APPEND)) == -1) 
                fprintf(stderr, "CAVISSON: Error in opening the url file\n");
            else {
                sprintf(buf, "%d", cavisson_doc_mod_fd);
                if (setenv("CAVISSON_FD", buf, 1) == -1)
                    cout << "CAVISSON: Error in seting env. variable" << endl;
            }
            unsetenv("CAVISSON_URL_FILE");
        }   
        
        if (setenv("CAVISSON_TOGGLE", "1", 1) == -1)
        cout << "CAVISSON: Error in setting env. variable" << endl;*/

        /*        if (db_create(&cavisson_hash_table, NULL, 0))
            cout << "CAVISSON: Error in creating new hashtable" << endl;
        
        if (cavisson_hash_table->set_h_hash(cavisson_hash_table, cavisson_hash_function))
            cout << "CAVISSON: Error in setting hash function" << endl;

        if (cavisson_hash_table->open(cavisson_hash_table, NULL, NULL, DB_HASH, 0, 0))
        cout << "CAVISSON: Error in opening hash table" << endl;*/

        /*cavisson_hash_table = dbopen(NULL, 0, 0, DB_HASH, NULL); */

        //        cavisson_hash_table = PL_NewHashTable(0, PL_HashString, PL_CompareStrings, PL_CompareValues, &CavissonAllocOps, NULL);
        cavisson_hash_table = PL_NewHashTable(0, PL_HashString, PL_CompareStrings, PL_CompareValues, NULL, NULL);

        data = (char*) malloc(2 * sizeof(char));
        sprintf(data, "1");
        
        key = (char*) malloc((strlen("NEXT_PAGE") + 1) * sizeof(char));
        sprintf(key, "NEXT_PAGE");
        
        if (PL_HashTableAdd(cavisson_hash_table, key, data))
            cout << "CAVISSON: Error in adding to hash table\n" << endl;
...
#define REPEAT_LENGTH 10
	//KQ: How  is this toggle env used
        if (getenv("CAVISSON_TOGGLE") && (!(strcmp(getenv("CAVISSON_TOGGLE"), "1")))) {
            int string_length;
            int truncate_num;
            char* hash_return;
            char repeat_value[REPEAT_LENGTH];
            int url_num = 1;
            char* key;
            char* data;
            int hash_size = 0;
            int temp_url_num;
            int free_key = 1;
            char page_func_str[1024];
            static char page_str[1024];
            char hash_define_str[1024];
            char hash_define_constant[1024];
	    char *sess_name;
            int i;

	    sess_name = getenv("CAVISSON_SESS_NAME");
	    if (strcmp (sess_name, last_cavisson_sess_name)) {
		strcpy(last_cavisson_sess_name,  sess_name);
		define_cnt = 0;
		if (cavisson_hash_table)
		     PL_HashTableDestroy(cavisson_hash_table);
            ...
            //if (cavisson_h_doc_mod_fd == -1) 
	    {
            ...
            if (getenv("CAVISSON_NUM_EMBED")) {
                ...
                if (write(cavisson_doc_mod_fd, page_end_buf, strlen(page_end_buf)) != strlen(page_end_buf))
                    cout << "CAVISSON: Error in writing to url file" << endl;

                if (write(cavisson_doc_mod_fd, "        next_page = check_page_", strlen("        next_page = check_page_")) != strlen("        next_page = check_page_"))
                    cout << "CAVISSON: Error in writing to url file" << endl;
                if ((last_page = getenv("CAVISSON_LAST_PAGE"))) {
                    if (write(cavisson_doc_mod_fd, last_page, strlen(last_page)) != strlen(last_page))
                        cout << "CAVISSON: Error in writing to url file" << endl;
                }
                if (write(cavisson_doc_mod_fd, "();\n        break;\n\n", strlen("();\n        break;\n\n")) != strlen("();\n        break;\n\n"))
                    cout << "CAVISSON: Error in writing to url file" << endl;
            }

	    //KQ: What's this check? Probably checking for != -1
            if (cavisson_doc_mod_fd && cavisson_det_doc_mod_fd) {
                nsCString main_url;
                char* page_name;
                char* page_name_ptr;

                aURI->GetSpec(main_url);
                if ((truncate_num = main_url.FindChar('?', -1, -1)) != -1)
                    main_url.Truncate(truncate_num);
                
                page_name = (char*) malloc(main_url.Length()+1);
                if (!page_name) {
                    fprintf(stderr, "Error in getting memory for the page name\n");
                    exit(-1);
                }
                
                page_name_ptr = page_name;
                
                strcpy(page_name, main_url.get());
                while (strchr(page_name_ptr, '/')) {
                    page_name_ptr = strchr(page_name_ptr, '/');
                    page_name_ptr++;
                }

                //if (strchr(page_name_ptr, '?'))
                    //strchr(page_name_ptr, '?')[0] = 0;

		strtok (page_name_ptr, "?;");
                if (strlen(page_name_ptr) == 0) {
                    free(page_name);
                    page_name = (char*) malloc(strlen("index") + 1);
                    if (!page_name) {
                        fprintf(stderr, "Error in reallocating memory for the page name\n");
                        exit(-1);
                    }
                    sprintf(page_name, "%s", "index");
                    page_name_ptr = page_name;
                }

                for (i = 0; i < strlen(page_name_ptr); i++) {
                    if ((!isalnum(page_name_ptr[i])) && (page_name_ptr[i] != '_'))
                        page_name_ptr[i] = '_';
                }
                
                if ((hash_return = (char*)PL_HashTableLookup(cavisson_hash_table, page_name_ptr))) {
                    url_num = atoi(hash_return);
                    free(hash_return);
                } else {
                    url_num = 1;
                    free_key = 0;
                ...
                data = (char*) malloc((hash_size + 1) * sizeof(char));
                sprintf(data, "%d", url_num + 1);
                
                key = (char*) malloc((strlen(page_name_ptr) + 1) * sizeof(char));
                sprintf(key, "%s", page_name_ptr);
                ...
                if (url_num > 1) {
                    memset(repeat_value, 0, REPEAT_LENGTH);
                    sprintf(repeat_value, "_%d", url_num);
                }

                if (write(cavisson_doc_mod_fd, "      case ", strlen("      case ")) != strlen("      case "))
                    cout << "CAVISSON: Error in writing to url file" << endl;
                if (write(cavisson_doc_mod_fd, page_name_ptr, strlen(page_name_ptr)) != strlen(page_name_ptr))
                    cout << "CAVISSON: Error in writing to url file" << endl;
                if (url_num > 1)
                    if (write(cavisson_doc_mod_fd, repeat_value, strlen(repeat_value)) != strlen(repeat_value))
                        cout << "CAVISSON: Error in writing to url file" << endl;
                if (write(cavisson_doc_mod_fd, ":\n        web_url (", strlen(":\n        web_url (")) != strlen(":\n        web_url ("))
                    cout << "CAVISSON: Error in writing to url file" << endl;
                if (write(cavisson_doc_mod_fd, page_name_ptr, strlen(page_name_ptr)) != strlen(page_name_ptr))
                ...
                if (write(cavisson_doc_mod_fd, ",\n", 2) != 2)
                    cout << "CAVISSON: Error in writing to url file" << endl;
                if (write(cavisson_det_doc_mod_fd, "--Page ", strlen("--Page ")) != strlen("--Page "))
                    cout << "CAVISSON: Error in writing to detail url file" << endl;
                if (write(cavisson_det_doc_mod_fd, page_name_ptr, strlen(page_name_ptr)) != strlen(page_name_ptr))
                    cout << "CAVISSON: Error in writing to detail url file" << endl;
                
                if (url_num > 1) {
                    if (write(cavisson_det_doc_mod_fd, repeat_value, strlen(repeat_value)) != strlen(repeat_value))
                        cout << "CAVISSON: Error in writing to detail url file" << endl;
                }
                ...
                /*                if (write(cavisson_doc_mod_fd, "\n", string_length) != string_length)
                                  cout << "CAVISSON: Error in writing to url file" << endl;*/
                if (write(cavisson_det_doc_mod_fd, "\n", string_length) != string_length)
                    cout << "CAVISSON: Error in writing to detail url file" << endl;
                if (setenv("CAVISSON_FIRST_URL", "1", 1) == -1)
                    cout << "CAVISSON: Error in setting env. variable" << endl;


                char* temp_ptr;
                for (i = 0, temp_ptr = page_name_ptr; *temp_ptr; temp_ptr++, i++) {
                    hash_define_constant[i] = toupper(*temp_ptr);
                }
                hash_define_constant[i] = '\0';

                if (url_num > 1) {
                    sprintf(hash_define_constant, "%s_%d", hash_define_constant, url_num);
                    sprintf(page_str, "%s_%d", page_name_ptr, url_num);
                } else {
                    sprintf(page_str, "%s", page_name_ptr);
                }

                //if (setenv("CAVISSON_LAST_PAGE", page_name_ptr, 1) == -1)
                if (setenv("CAVISSON_LAST_PAGE", page_str, 1) == -1)
                    cout << "CAVISSON: Error in setting env. variable" << endl;

                //sprintf(page_func_str, "\n\treturn %s;\n}\n\nint check_page_%s(void) {\n", hash_define_constant, page_str);
                //sprintf(hash_define_str, "#define %s %d\n", hash_define_constant, define_cnt++);
                sprintf(page_func_str, "\n\treturn %s;\n}\n\nint check_page_%s(void) {\n", page_str, page_str);
                sprintf(hash_define_str, "#define %s %d\n", page_str, define_cnt++);

                if (write(cavisson_c_doc_mod_fd, page_func_str, strlen(page_func_str)) != strlen(page_func_str))
                    cout << "CAVISSON: Error in writing to c file" << endl;
                
                if (write(cavisson_h_doc_mod_fd, hash_define_str, strlen(hash_define_str)) != strlen(hash_define_str))
                    cout << "CAVISSON: Error in writing to h file" << endl;
