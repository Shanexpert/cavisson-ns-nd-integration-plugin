...
void
nsHttpHeaderArray::ShortFlatten(nsACString &buf, int cap_fd, int det_fd)
{
    PRInt32 i, count = mHeaders.Count();
    ...
    for (i=0; i<count; ++i) {
        nsEntry *entry = (nsEntry *) mHeaders[i];
        if (!strcmp(entry->header.get(), "User-Agent"))
            continue;
        if (!strcmp(entry->header.get(), "Keep-Alive"))
            continue;
        if (!strcmp(entry->header.get(), "Connection"))
            continue;
        if (!strcmp(entry->header.get(), "Accept"))
            continue;
        if (!strcmp(entry->header.get(), "Accept-Encoding"))
            continue;
        if (!strcmp(entry->header.get(), "Referer"))
            continue;
        if (!strcmp(entry->header.get(), "Content-Length"))
            continue;
            
        if (cap_fd != -1) {
            if (!strcmp(entry->header.get(), "Host"))
                continue;
            if (!strcmp(entry->header.get(), "Cookie")) {
                if (write(det_fd, "COOKIE=", strlen("COOKIE=")) != strlen("COOKIE="))
                    cout << "CAVISSON: error in writing to detail url file" << endl;
                cookie_start = entry->value.get();
                while (cookie_start) {
                    cookie_end = strchr(cookie_start, '=');
                    if (!cookie_end) {
                        cout << "CAVISSON: error in reading cookies" << endl;
                        break;
                    }
                    if (write(det_fd, cookie_start, cookie_end - cookie_start) != (cookie_end - cookie_start))
                        cout << "CAVISSON: error in writing to detail url file" << endl;
                    if (write(det_fd, ";", 1) != 1)
                        cout << "CAVISSON: error in writing to detail url file" << endl;
                    cookie_start = strchr(cookie_end, ';');
                }
                if (write(det_fd, "\n", 1) != 1)
                    cout << "CAVISSON: error in writing to capture file" << endl;
            } else {
                if (write(cap_fd, "          HEADER=", strlen("          HEADER=")) != strlen("          HEADER="))
                    cout << "CAVISSON: error in writing to capture file" << endl;
                string_length = strlen(entry->header.get());
                if (write(cap_fd, entry->header.get(), string_length) != string_length)
                    cout << "CAVISSON: error in writing to capture file" << endl;
                if (write(cap_fd, ": ", 2) != 2)
                    cout << "CAVISSON: error in writing to capture file" << endl;
                string_length = strlen(entry->value.get());
                if (write(cap_fd, entry->value.get(), string_length) != string_length)
                    cout << "CAVISSON: error in writing to capture file" << endl;
                if (write(cap_fd, ",\n", 2) != 2)
                    cout << "CAVISSON: error in writing to capture file" << endl;
            }
